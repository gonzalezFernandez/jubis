...
const CHARS = {
  muchaga: {
    ...
    upgrades: [
      ...
      { id:'gintonic', name:'Gin-Tonic Premium',    icon:'🍸', desc:'Con pepino y todo. Aquí somos finos.',               cost:2200,  prod:10,   cbonus:4,   base:2200  },
      { id:'barril',   name:'Barril de Estrella',   icon:'🛢️', desc:'Cien litros de pura felicidad ajena.',              cost:12000, prod:50,   cbonus:12,  base:12000 },
      { id:'cocktail', name:'Cócteles Artesanales', icon:'🍹', desc:'Inventos raros que cobras el doble.',                cost:50000, prod:200,  cbonus:40,  base:50000 },
      { id:'maquina',  name:'Máquina Tragaperras',  icon:'🎰', desc:'Pa los clientes más desesperados del barrio.',       cost:200000,prod:500,  cbonus:90,  base:200000},
    ],
    msgs:['¿Qué va a ser, jefe?','Hoy estamos de oferta en los combinados.','El JB se acaba, hay Dyc de sobra.','Ese del fondo lleva aquí desde las 10 de la mañana.','Con esto aguanto hasta el amanecer.','La barra no se limpia sola... bueno, a veces sí.','WHISKY... perdón. ¿Qué decías?','Otro día más en el paraíso, tío.','El barril vacío es señal de una noche exitosa.'],
  },
...
const ACHIEVEMENTS = {
  muchaga: [
    ...
    { id:'gintonic_king',  icon:'🍸', name:'Del Grifo al Gin',    desc:'Compra el Gin-Tonic Premium. Aspiraciones.',                  cond: s => (s.upgrades.gintonic||0) >= 1 },
    { id:'maquina',        icon:'🎰', name:'El Traga Tragado',    desc:'Instala la máquina tragaperras. Negocio dentro del negocio.', cond: s => (s.upgrades.maquina||0) >= 1 },
...
const UNLOCK_ORDER = ['muchaga', 'noah', 'fita', 'nanduko', 'extraperlo'];
...
function goBack() {
  ...
  stopGame();
  [S.chicaTimer, S.policeTimer, S.rocaTimer, S.fightTimer, S.banyoTimer, S.raicesTimer].forEach(t => clearTimeout(t));
