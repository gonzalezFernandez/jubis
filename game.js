...
const ACHIEVEMENTS = {
  muchaga: [
    { id:'primera_ronda',  icon:'☕', name:'Primera Ronda',         desc:'50 cañas. Te tiemblan las manos pero el grifo no para.',                     cond: s => s.totalCurrency >= 50 },
    { id:'grifo_no_para',  icon:'🍺', name:'El Grifo No Para',      desc:'Primer grifo instalado. Ahora sí hay excusa para no cerrar.',                cond: s => (s.upgrades.cana||0) >= 1 },
    { id:'maestro_bar',    icon:'🥃', name:'Maestro del Bar',       desc:'500 cañas. Ya hueles a bar desde 3 metros.',                                 cond: s => s.totalCurrency >= 500 },
    { id:'primera_pelea',  icon:'🥊', name:'Primer Puñetazo',       desc:'Primer altercado. Para un jueves es normalito.',                             cond: s => (s.achData.fightWins||0) + (s.achData.fightLosses||0) >= 1 },
    { id:'campeon_barra',  icon:'💪', name:'Campeón de la Barra',   desc:'5 peleas ganadas. Muchaga no es guapo pero da miedo.',                        cond: s => (s.achData.fightWins||0) >= 5 },
    { id:'whyshkeys_1',    icon:'🤯', name:'¡WHISKY!',              desc:'Primera vez. Inevitable. Trágico. Inevitable.',                               cond: s => (s.achData.wskActivations||0) >= 1 },
    { id:'whyshkeys_win',  icon:'🃏', name:'El Dominó de la Paz',   desc:'Primero en ganar una pelea. Autoridad moral cuestionable.',                  cond: s => (s.achData.fightWins||0) >= 1 },
  ...
  fita: [
    { id:'primera_birra',  icon:'🍺', name:'Primera Birra',         desc:'50 vicios. La noche empieza y el cerebro ya avisa.',                         cond: s => s.totalCurrency >= 50 },
    { id:'porro_medic',    icon:'💚', name:'El Porro Medicinal',     desc:'CBD comprado. Para el estrés, claro. Claro.',                                cond: s => (s.upgrades.cbd||0) >= 1 },
    { id:'dobletazo_1',    icon:'⚡', name:'¡DOBLETAZO!',            desc:'Primer dobletazo. El material funciona mejor de lo esperado.',                cond: s => (s.achData.dobletazos||0) >= 1 },
    { id:'dobletazo_10',   icon:'⚡⚡',name:'Material de Calidad',   desc:'10 dobletazos. El hígado ya ha presentado el finiquito.',                     cond: s => (s.achData.dobletazos||0) >= 10 },
    ...
    { id:'triplete',       icon:'🚀', name:'¡TRIPLETE!',             desc:'3 TRIPLETES. Fita en modo bestia, que es mucho decir.',                       cond: s => (s.achData.yeyoWins||0) >= 3 },
    ...
    { id:'yeyo_pro',       icon:'🏔️', name:'El del Montón Más Gordo',desc:'10 tripletes. Fita ya es un referente en el gremio. Lamentablemente.',        cond: s => (s.achData.yeyoWins||0) >= 10 },
  ...
  noah: [
    { id:'primer_like',    icon:'❤️', name:'Primer Like',            desc:'50 likes. El algoritmo te sonríe. Por ahora.',                               cond: s => s.totalCurrency >= 50 },
    { id:'primer_flirt',   icon:'💬', name:'Primer Flirteo',         desc:'Primer intento de ligue. Lo importante es intentarlo, dice Noah.',            cond: s => (s.achData.chicaTotal||0) >= 1 },
    { id:'lothario',       icon:'💘', name:'Lothario del Barrio',    desc:'Liga 3 veces. Noah tiene su público, aunque sea pequeño.',                    cond: s => (s.achData.chicaLiga||0) >= 3 },
    { id:'rechazado',      icon:'😬', name:'Los Mocos No Paran',     desc:'Rechazado 5 veces. La estadística no miente aunque duela.',                   cond: s => (s.achData.chicaFails||0) >= 5 },
    { id:'farmacia',       icon:'💊', name:'Cliente Habitual',       desc:'Primera visita a la farmacia. El farmacéutico ya suspira al verte.',           cond: s => (s.achData.farmacia||0) >= 1 },
    { id:'tiktok_addict',  icon:'🎵', name:'TikTok Addict',          desc:'TikTok Viral comprado. Tres horas pasan en diez segundos, Noah.',              cond: s => (s.upgrades.tiktok||0) >= 1 },
    { id:'verificado',     icon:'✅', name:'Verificado',             desc:'Palomita azul conseguida. Igual de solo pero con más likes.',                  cond: s => (s.upgrades.tick||0) >= 1 },
    { id:'sin_std',        icon:'🧬', name:'Dos Veces Curado',       desc:'2 visitas a la farmacia. Las consecuencias del ligue sin criterio se acumulan.',cond: s => (s.achData.farmacia||0) >= 2 },
    { id:'viral',          icon:'🚀', name:'¡VIRAL!',                desc:'5000 likes. Noah el fenómeno digital. Por estas tierras.',                     cond: s => s.totalCurrency >= 5000 },
    { id:'influencer_loc', icon:'⭐', name:'Influencer Local',       desc:'Famoso en 3 km² de radio. La fama tiene un precio bajo.',                      cond: s => (s.upgrades.influencer||0) >= 1 },
    { id:'rey_scroll',     icon:'👑', name:'Rey del Scroll',         desc:'50000 likes. Fenómeno digital. La madre de Noah lo ha compartido todo.',       cond: s => s.totalCurrency >= 50000 },
    { id:'socorrista',     icon:'🏊', name:'Socorrista de Salinas',  desc:'Primer rescate. La Voz de Asturias te pone en portada. Una vez.',              cond: s => (s.achData.rescates||0) >= 1 },
    { id:'primera_plana',  icon:'📰', name:'Primera Plana',          desc:'3 rescates. Mamá ha recortado los artículos y tiene un altar.',                cond: s => (s.achData.rescates||0) >= 3 },
    { id:'farmacia_vip',   icon:'🏥', name:'Bono Fidelidad',         desc:'3 visitas a la farmacia. El farmacéutico te llama por tu nombre de pila.',     cond: s => (s.achData.farmacia||0) >= 3 },
    { id:'sin_filtro',     icon:'🤳', name:'Sin Filtro Porque No Hace Falta', desc:'2000 likes. Convéncete tú mismo, Noah.',                             cond: s => s.totalCurrency >= 2000 },
    { id:'dating_app',     icon:'💔', name:'App Desinstalada',       desc:'10 intentos de ligue. La app te ha cobrado extra por el volumen.',             cond: s => (s.achData.chicaTotal||0) >= 10 },
    { id:'vocacion_socorr',icon:'🌊', name:'Vocación de Socorrista', desc:'5 rescates. Salinas te paga el bono de verano en créditos del chiringuito.',   cond: s => (s.achData.rescates||0) >= 5 },
    { id:'super_viral',    icon:'🔥', name:'Super Viral Asturias',   desc:'250000 likes. Famoso en toda Asturias. En Oviedo te reconocen dos personas.',  cond: s => s.totalCurrency >= 250000 },
  ],
  diego: [
    { id:'primera_palabra',  icon:'💬', name:'Primera Chapa',          desc:'50 palabras. El rollo ha comenzado y no hay botón de pausa.',                 cond: s => s.totalCurrency >= 50 },
    { id:'modo_yapping',     icon:'🔥', name:'Modo Yapping',            desc:'Chapa al 80%. Diego en modo cháchara total, que es decir mucho.',             cond: s => (s.achData.chapaMax||0) >= 80 },
    { id:'yapping_supremo',  icon:'🎙️', name:'Yapping Supremo',        desc:'Primer Yapping Supremo. ×5. El resto de la mesa abandona la mesa.',           cond: s => (s.achData.yappingSupremos||0) >= 1 },
    { id:'primer_piso',      icon:'🏠', name:'Primer Piso Visitado',    desc:'Primera alerta de Idealista. Llegas antes que el agente.',                    cond: s => (s.achData.pisosVisitados||0) >= 1 },
    { id:'salinas_premium',  icon:'⭐', name:'¡Salinas!',               desc:'Un piso en Salinas. La ostia en verso.',                                      cond: s => (s.achData.salinasVisitados||0) >= 1 },
    { id:'cerdo_trufero',    icon:'🐷', name:'Cerdo Trufero',           desc:'3 olas derechas. Solo derechas, como Dios manda y Diego vota.',               cond: s => (s.achData.olasDerechas||0) >= 3 },
    { id:'mal_humor',        icon:'😤', name:'Mal Humor',               desc:'Cogiste una izquierda. Diego en silencio de rabia.',                          cond: s => (s.achData.olasIzquierdas||0) >= 1 },
    { id:'silencio',         icon:'😶', name:'Silencio Incómodo',       desc:'Diego pierde el hilo habiendo llegado al 50%. Brutal.',                       cond: s => (s.achData.silenciosIncomodos||0) >= 1 && (s.achData.chapaMax||0) >= 50 },
    { id:'hipotecado',       icon:'🏦', name:'Hipotecado',              desc:'Hipoteca firmada. 30 años de compromiso que Idealista ya sabe.',               cond: s => (s.upgrades.hipoteca||0) >= 1 },
    { id:'idealista_pro',    icon:'📲', name:'Idealista Pro',           desc:'5 pisos visitados. El agente ya te llama sin haber publicado.',                cond: s => (s.achData.pisosVisitados||0) >= 5 },
    { id:'monopolio_castri', icon:'🏆', name:'Monopolio Castrillón',    desc:'250000 palabras. Todo Castrillón sabe quién es Diego y lo evita.',             cond: s => s.totalCurrency >= 250000 },
    { id:'yapping_5',        icon:'📢', name:'El Insufrible',           desc:'5 Yapping Supremos. Diego puede irse a la mierda y volver a tiempo para más.',cond: s => (s.achData.yappingSupremos||0) >= 5 },
    { id:'silencio_3',       icon:'💀', name:'El Mudo de la Peña',      desc:'3 silencios incómodos. La gente ya trae auriculares cuando viene Diego.',     cond: s => (s.achData.silenciosIncomodos||0) >= 3 },
    { id:'pisos_10',         icon:'🏙️', name:'El Agente Inmobiliario', desc:'10 pisos visitados. El agente te da comisión en recochineo.',                  cond: s => (s.achData.pisosVisitados||0) >= 10 },
    { id:'olas_5d',          icon:'🌊', name:'Derecha Siempre',         desc:'5 olas derechas. Diego solo gira a la derecha. En todo sentido.',              cond: s => (s.achData.olasDerechas||0) >= 5 },
    { id:'callar_1',         icon:'🤫', name:'Una Vez en Su Vida',      desc:'Para al interruptor en zona perfecta. Primera y única vez que calla bien.',   cond: s => (s.achData.interrupciones||0) >= 1 },
    { id:'callar_5',         icon:'🎯', name:'El Experto del Silencio', desc:'5 veces en zona perfecta. Diego ha aprendido cuándo callar. Tarde, pero bien.',cond: s => (s.achData.interrupciones||0) >= 5 },
    { id:'aviles_trauma',    icon:'🏚️', name:'Trauma de Villacloaca',  desc:'Visita un piso en Avilés. Ojos que vieron lo que vieron.',                    cond: s => (s.achData.avilesVisitados||0) >= 1 },
  ],
  weeman4k: [
    { id:'primera_toma',     icon:'🎬', name:'Primera Toma',           desc:'50 tomas. El rodaje ha empezado y Coopers ya está en el encuadre.',            cond: s => s.totalCurrency >= 50 },
    { id:'reflex',           icon:'📷', name:'Ojo Artístico',          desc:'Réflex comprada. Ahora sí que eres fotógrafo, según tú.',                      cond: s => (s.upgrades.camara_foto||0) >= 1 },
    { id:'sin_manchar',      icon:'💩', name:'Sin Manchar',            desc:'5 cagadas evitadas. La acera sí, tú no. Por esta vez.',                        cond: s => (s.achData.cagadasEvitadas||0) >= 5 },
    { id:'looteo',           icon:'🍕', name:'Del Mantel Ajeno',       desc:'Primer looteo de Coopers. El perro tiene más iniciativa que el dueño.',         cond: s => (s.achData.looteos||0) >= 1 },
    { id:'triskel',          icon:'🐾', name:'Triskel',                desc:'3 lavados de Coopers. El Triskel ya te reconoce por el olor.',                  cond: s => (s.achData.jamones||0) >= 3 },
    { id:'perfect_1',        icon:'💥', name:'¡PERFECT!',              desc:'Primer PERFECT. La diarrea como forma de arte contemporáneo.',                  cond: s => (s.achData.perfectHits||0) >= 1 },
    { id:'ultra',            icon:'⚡', name:'ULTRAPERFECT',           desc:'Primer ULTRAPERFECT. El mundo del surf documental nunca fue igual.',             cond: s => (s.achData.ultraActivations||0) >= 1 },
    ...
    { id:'ultra_3',          icon:'⚡⚡',name:'Modo Dios',              desc:'3 ULTRAPERFECTS. El universo del surf asturiano no volverá a ser el mismo.',    cond: s => (s.achData.ultraActivations||0) >= 3 },
    { id:'mancha_total',     icon:'💩💩',name:'Coleccionista de Mierda',desc:'Pisas 5 cagadas. El Triskel ya te huele desde dos calles.',                     cond: s => (s.achData.cagadasRecogidas||0) >= 5 },
  ],
  nanduko: [
    { id:'primer_trato',     icon:'🤝', name:'Primer Trato',           desc:'50 pasta. El negocio ha empezado. Lo de la legalidad ya se verá.',              cond: s => s.totalCurrency >= 50 },
    { id:'reunion_banyo',    icon:'🚽', name:'Reunión de Negocios',    desc:'Primera reunión en el baño. El entorno no importa si el negocio mola.',         cond: s => (s.achData.banyoWins||0) >= 1 },
    { id:'banyo_pro',        icon:'🤫', name:'Discreto Total',         desc:'3 reuniones en el baño. Más reuniones que el banco de enfrente.',                cond: s => (s.achData.banyoWins||0) >= 3 },
    { id:'primera_huida',    icon:'🏃', name:'Primera Huida',          desc:'Primera escapada de la poli. Pies que os quiero.',                               cond: s => (s.achData.policeEscapes||0) >= 1 },
    { id:'sin_rastro',       icon:'👻', name:'Sin Rastro',             desc:'5 escapadas. Nanduko es como el humo, solo que huele peor.',                     cond: s => (s.achData.policeEscapes||0) >= 5 },
    { id:'pillado',          icon:'👮', name:'Experiencias Formativas',desc:'3 veces pillado. Parte del proceso, dicen. El proceso duele.',                   cond: s => (s.achData.policeCaptures||0) >= 3 },
    { id:'red_activa',       icon:'🗺️', name:'Red Activa',             desc:'Red de distribución comprada. Empresario nato. Sin declarar.',                   cond: s => (s.upgrades.red||0) >= 1 },
    { id:'colombia',         icon:'✈️', name:'Viaje de Negocios',      desc:'Contacto colombiano activado. Mensual y sin factura.',                           cond: s => (s.upgrades.colombia||0) >= 1 },
    { id:'raices_escape',    icon:'🛤️', name:'Conozco el Camino',      desc:'Primera escapada en Raíces. El terreno es tuyo aunque no lo sepan.',             cond: s => (s.achData.raicesEscapes||0) >= 1 },
    { id:'raices_pro',       icon:'🏎️', name:'Piloto de Raíces',       desc:'5 escapadas sin que te pillen. La guardia civil ya ni sale.',                   cond: s => (s.achData.raicesEscapes||0) >= 5 },
    { id:'magnate',          icon:'💰', name:'Magnate',                desc:'5000 pasta. El barrio ya huele distinto cuando pasa Nanduko.',                   cond: s => s.totalCurrency >= 5000 },
    { id:'pablo',            icon:'🦁', name:'Pablo Nanduko',          desc:'50000 pasta. El cartel más temido del club. Y del municipio.',                   cond: s => s.totalCurrency >= 50000 },
    { id:'banyo_5',          icon:'🚿', name:'Gerente del Aseo',       desc:'5 reuniones en el baño. Tu empresa factura más que la del ayuntamiento.',        cond: s => (s.achData.banyoWins||0) >= 5 },
    { id:'banyo_10',         icon:'🏛️', name:'El Despacho',            desc:'10 reuniones en el baño. Has dignificado el servicio masculino del bar.',        cond: s => (s.achData.banyoWins||0) >= 10 },
    { id:'pillado_5',        icon:'🚔', name:'Fichado en Comisaría',   desc:'5 veces pillado. Tienes expediente, abogado de oficio y foto del DNI enmarcada.',cond: s => (s.achData.policeCaptures||0) >= 5 },
    { id:'raices_legend',    icon:'🌫️', name:'El Fantasma de Raíces',  desc:'10 escapadas. La guardia te ha puesto nombre en clave: El Inasible.',           cond: s => (s.achData.raicesEscapes||0) >= 10 },
    { id:'cien_mil',         icon:'💎', name:'Cien de las Grandes',    desc:'100000 pasta. Hacienda no lo sabe. Y así va a seguir.',                          cond: s => s.totalCurrency >= 100000 },
    { id:'doscientos_50k',   icon:'🏦', name:'El Magnate de Avilés',   desc:'250000 pasta. Castrillón te mira con envidia. Hasta Salinas.',                  cond: s => s.totalCurrency >= 250000 },
  ],
  extraperlo: [
    { id:'bienvenido',       icon:'🎪', name:'Bienvenido al Caos',     desc:'Primer click en modo Extraperlo. Sin retorno. Bienvenido al infierno.',         cond: s => s.totalClicks >= 1 },
    { id:'roca_1',           icon:'🤜', name:'¡Roca Pintada!',         desc:'Primera vez que les pegas. Por el arte y por la humanidad.',                    cond: s => (s.achData.rocaHits||0) >= 1 },
    { id:'roca_5',           icon:'🎸', name:'Enemigo del Rock',       desc:'5 veces con Roca Pintada. Son muy persistentes para lo malos que son.',         cond: s => (s.achData.rocaHits||0) >= 5 },
    { id:'vasos_1',          icon:'🧼', name:'Limpiabarras',           desc:'Primer lavado de vasos. La dignidad tiene un suelo y tú lo has encontrado.',    cond: s => (s.achData.vasosLimpiados||0) >= 1 },
    { id:'vasos_5',          icon:'🗑️', name:'Del Contenedor',         desc:'5 rondas de vasos. El olfato ya es un recuerdo del pasado.',                   cond: s => (s.achData.vasosLimpiados||0) >= 5 },
    { id:'triplete_xp',      icon:'⚡', name:'Triplete Extraperlo',    desc:'Primer TRIPLETE en modo caos total. Fita estaría orgulloso y asustado.',        cond: s => (s.achData.yeyoWins||0) >= 1 },
    { id:'sin_vasos',        icon:'🧹', name:'Ni un Vaso en Pie',      desc:'10 rondas de vasos. El suelo ya no existe. Solo vasos.',                        cond: s => (s.achData.vasosLimpiados||0) >= 10 },
    { id:'roca_10',          icon:'🎤', name:'Técnicas y Procedimientos',desc:'10 veces con Roca Pintada. Ya os conocéis demasiado bien. Es triste.',         cond: s => (s.achData.rocaHits||0) >= 10 },
    { id:'caos_5k',          icon:'🌀', name:'¡Otra Ronda!',           desc:'5000 caos. El festival no para. El hígado tampoco pero de otra manera.',        cond: s => s.totalCurrency >= 5000 },
    { id:'pase_vip',         icon:'🏷️', name:'Pase VIP Conseguido',   desc:'Pase VIP en tu poder. Acceso a casi todo, por si acaso.',                       cond: s => (s.upgrades.pase||0) >= 1 },
    { id:'leyenda_xp',       icon:'🏆', name:'Leyenda del Extraperlo', desc:'50000 caos. Técnicas y Procedimientos te piden una foto y tú la cobras.',       cond: s => s.totalCurrency >= 50000 },
    { id:'primer_rescate',   icon:'🏊', name:'Primer Rescate',         desc:'Salvas a alguien. Héroe de festivales. Brevemente.',                             cond: s => (s.achData.rescates||0) >= 1 },
    { id:'derechazo_xp',     icon:'🌊', name:'Derechazo en el Caos',   desc:'Primera ola derecha en el Extraperlo. Solo derechas aquí.',                     cond: s => (s.achData.olasDerechas||0) >= 1 },
    { id:'dos_orillas',      icon:'🏄', name:'Las Dos Orillas',        desc:'Una derecha y una izquierda. Eres el más democrático del festival.',             cond: s => (s.achData.olasDerechas||0) >= 1 && (s.achData.olasIzquierdas||0) >= 1 },
...
function clickIdealista() {
  ...
  if (piso.zona === 'Avilés') {
    S.achData.avilesVisitados = (S.achData.avilesVisitados || 0) + 1;
